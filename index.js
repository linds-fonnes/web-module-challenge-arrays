...
function addFlavor(addToThisArray,addThisFlavor){
  addToThisArray.unshift(addThisFlavor);
...
function getFlavorByIndex(whichArray,indexNumber){
    return whichArray[indexNumber];
}
...
function removeFlavorByName(arrayToChange,flavorToRemove){
    const indexNumber = arrayToChange.indexOf(flavorToRemove);
    arrayToChange.splice(indexNumber,1);
    return arrayToChange;
}
...
function filterByWord(arrayToFilter,wordToFilter){
    let filteredArray = [];
    for(let i=0; i < arrayToFilter.length; i++){
        if(arrayToFilter[i].includes(wordToFilter)){
